            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-semibold">Posts with tag: {{ $tag->name }}</h3>
                        
                        @can('admin')
                            <div class="flex space-x-2">
                                <a href="{{ route('tags.edit', $tag->id) }}" class="px-3 py-1 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
                                    Edit
                                </a>
                                ...
                                <form action="{{ route('tags.destroy', $tag->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-3 py-1 bg-red-500 text-white rounded-md hover:bg-red-600" onclick="return confirm('Are you sure you want to delete this tag?')">
                                        Delete
                                    </button>
                                </form>
                            </div>
                                            ...
                                            @if($post->tags->count() > 0)
                                                <div class="flex flex-wrap gap-2 mb-2">
                                                    @foreach($post->tags as $postTag)
                                                        <span class="px-2 py-1 bg-gray-100 text-xs rounded-full text-gray-600 {{ $postTag->id === $tag->id ? 'bg-orange-100 text-orange-600 border border-orange-300' : '' }}">
                                                            {{ $postTag->name }}
                                                        </span>
                                                    @endforeach
                                                </div>
